...
import com.ggkbigdata.fire.model.pdfparse.Cover;
import com.ggkbigdata.fire.model.pdfparse.ListResult;
import com.ggkbigdata.fire.model.pdfparse.PDFParserResult;
import com.ggkbigdata.fire.model.pdfparse.Result;
import com.ggkbigdata.fire.servie.PDFParserService;

/**
 *
 * @author lcc (lincc@ggkbigdata.com)
 * @version 1.0, 2017年2月21日 下午3:12:26
 */
@Service
public class PDFParserServiceImpl implements PDFParserService {
    ...
    public Cover processOnCover(String paragraph) {
        ...
        for(int i=0;i<lines.length;i++){
            ...
            if(m.find()){
                cover.setProjectName(m.group(1).trim());
                projectNameLine = i;
                continue;
            }
            m = projectAddress.matcher(line);
            if(m.find()){
                cover.setProjectAddress(m.group(1).trim());
                projectAddrLine = i;
                continue;
            }
            m = agentName.matcher(line);
            if(m.find()){
                cover.setAgentName(m.group(1).trim());
                continue;
            }
            m = qaName.matcher(line);
            if(m.find()){
                cover.setQaName(m.group(1).trim());
                continue;
            }
            m = reportNum.matcher(line);
            if(m.find()){
                cover.setReportNum(m.group(1).trim());
                continue;
            }
            m = qaAddress.matcher(line);
            if(m.find()){
                cover.setQaAddress(m.group(1).trim());
                continue;
            }
            m = contactTel.matcher(line);
            if(m.find()){
                cover.setContactTel(m.group(1).trim());
                continue;
            }
            m = contactFax.matcher(line);
            if(m.find()){
                cover.setContactFax(m.group(1).trim());
                continue;
            }
            m = contactPostcode.matcher(line);
            if(m.find()){
                cover.setContactPostcode(m.group(1).trim());
                continue;
            }
        }
